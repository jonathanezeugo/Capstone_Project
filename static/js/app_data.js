...
function buildTable() {
    d3.csv("static/data/Capstone_Data.csv").then(function(oilData) {

        // var yearFilter = parseInt($("#year").val()); 
        // var quarterFilter = parseInt($("#quarter").val());
        ...
        // apply filters
        var filteredData = oilData

        // if (yearFilter) {
        //     filteredData = filteredData.filter(row => parseInt(row.year) === (yearFilter));
        //     }
        // if (quarterFilter) {
        //     filteredData = filteredData.filter(row => parseInt(row.quarter) === (quarterFilter))
        //     } 
        // if (latitudeFilter) {
...
// function buildTableString(oilData) {
...
//     var datarows = oilData.map(x => [x["watch"], x["division"], x["district"], x["month1"],  x["day1"], x["status"], x["nibrs_crime_category"] ,x["hour"],x["latitude"],x ["longitude"]])
